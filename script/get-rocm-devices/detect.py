...
def get_gpu_info():
    ...
    for i in range(num_gpus):
        gpu_info = {
            ...
            "GPU Name": i,
            "GPU compute capability": f"{hip.hipDeviceComputeCapability(i)[0]}.{hip.hipDeviceComputeCapability(i)[1]}",
            "ROCM driver version": f"{hip.hipDriverGetVersion()[1]}",
            "ROCM runtime version": hip.hipRuntimeGetVersion()[1],
            "Global memory": hip.hipDeviceTotalMem(i)[1],
            "Max clock rate": f"{hip.hipDeviceGetAttribute(hip.hipDeviceAttribute_t(hipDeviceAttributeClockRate), i)[1]} MHz",
            "Total amount of shared memory per block": f"{hip.hipDeviceGetAttribute(hip.hipDeviceAttribute_t(hipDeviceAttributeMaxSharedMemoryPerBlock), i)[1]}",
            "Total number of registers available per block": f"{hip.hipDeviceGetAttribute(hip.hipDeviceAttribute_t(hipDeviceAttributeMaxRegistersPerBlock), i)[1]}",
