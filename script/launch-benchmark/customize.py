...
#                    dimension_values.append(value)
    ...
    for s in selection:
        ...
        if len(dimensions) == 0:
            row['test'] = '<a href="{}" target="_blank">{}</a>'.format(
                url, name)
        else:
        ...
        if s.get('functional', False):
            x = '<center><a href="{}" target="_blank"><img src="{}" height="32"></a></center>'.format(
                url, badges_url['functional'])
        row['functional'] = x
...
def gui(i):
    ...
    if uid != '':
        if len(lst) ==0:
            ...
            st.markdown(
    'Warning: More than 1 CM test found with UID "{}" - ambiguity!'.format(uid))
            return {'return': 0}
    ...
    if uid == '':
        ...
        if len(selection) >1:
            ...
            test_tags = st.text_input('Found {} CM tests. Prune them by tags:'.format(
                str(len(selection))), value=test_tags, key='test_tags').strip()
