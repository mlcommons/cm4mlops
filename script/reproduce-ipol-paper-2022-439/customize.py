...
def preprocess(i):
    ...
    if input_file_1 == '':
        input_file_1 = 'ipol-paper-2024-439-sample-image-1.png'

    if not os.path.isfile(input_file_1):
        return {'return': 1,
                'error': 'input file 1 "{}" not found'.format(input_file_1)}

    env['CM_INPUT_1'] = os.path.abspath(input_file_1)

    input_file_2 = env.get('CM_INPUT_2', '')
    if input_file_2 == '':
        input_file_2 = 'ipol-paper-2024-439-sample-image-2.png'

    if not os.path.isfile(input_file_2):
        return {'return': 1,
                'error': 'input file 2 "{}" not found'.format(input_file_2)}
